...
from evaluation import fold_orchestrator_multiuser as fold_orchestrator
from evaluation.metrics_llm_multiuser import EMOTION_COLUMNS, load_or_compute_fold_metrics
...
def _render_overview() -> None:
    ...
    st.caption(
        "Method: leave-2-songs-out few-shot in-context learning restricted to songs rated by at least 2 users. "
        "Each fold uses 5 training songs and 2 test songs, stopping once all eligible users appear in test songs."
    )
    ...
    with headline[3]:
        st.metric("Eligible Songs", state.get("eligible_song_count", 0))
    with headline[4]:
    ...
    with controls[0]:
        if st.button("Run Next Fold", use_container_width=True, key="multiuser_run_next_fold"):
            try:
    ...
    if completed_folds:
        ...
        for fold_number in completed_folds:
            ...
            with st.container():
                st.markdown(f"### Fold {fold_number}: multi-rater slice")
                meta = st.columns(6)
                with meta[0]:
                ...
                with meta[2]:
                    st.metric("Eligible Songs", summary.get("eligible_song_count", 0))
                with meta[3]:
                ...
                st.caption(
                    "Artifacts are stored under `llm analysis/`. "
...
def _render_fold_review() -> None:
    ...
    st.caption(
        "Only songs with at least 2 unique user ratings are included. "
        "Each fold scores the 2-song test slice against the average user ratings for those songs."
    )
    ...
    for fold_number in completed_folds:
        ...
        st.markdown(f"### Fold {fold_number}: 5 train songs / 2 test songs")
        summary_cols = st.columns(5)
        with summary_cols[0]:
            st.metric("Test Users", len(summary.get("test_users", [])))
        with summary_cols[1]:
            st.metric("Test Songs", summary.get("song_count", 0))
        with summary_cols[2]:
            st.metric("Eligible Songs", summary.get("eligible_song_count", 0))
        with summary_cols[3]:
...
def _render_model_comparison() -> None:
    ...
    st.caption(
        "Comparison table for one saved target song from the multi-rater workflow. "
        "Raw JSON is loaded from `llm analysis/data/raw_outputs`."
    )

    fold_number = st.selectbox("Select Fold", completed_folds, key="multiuser_comparison_fold")
    summary = _fold_summary(fold_number)
    ...
    selected_song = st.selectbox("Select Song", songs, key="multiuser_comparison_song")
...
def render(page_name: str, title: str = "LLM Analysis > 2 Users", caption: str | None = None):
    st.header(title)
    st.caption(
        caption
        or (
            "Separate artifact root: `llm analysis/`. "
            "This workflow includes only songs rated by at least 2 unique users."
        )
    )

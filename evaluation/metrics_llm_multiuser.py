...
from evaluation import fold_orchestrator_multiuser as workflow
...
ANNOTATIONS_DIR = workflow.ANNOTATIONS_DIR
GROUND_TRUTH_PATH = workflow.GROUND_TRUTH_PATH
LLM_ANALYSIS_DIR = workflow.LLM_ANALYSIS_DIR
EMOTION_COLUMNS = workflow.EMOTION_COLUMNS
...
def _load_ground_truth() -> dict:
    with GROUND_TRUTH_PATH.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    return {
        workflow._normalize_song_key(row["filename"]): {
...
def compute_fold_metrics(fold_number: int) -> dict:
    ...
    return {"fold": fold_number, "annotators": annotators, "comparisons": comparisons}
...
def compute_all_folds_metrics() -> dict:
    fold_results = []
    for row in workflow.get_fold_status():
